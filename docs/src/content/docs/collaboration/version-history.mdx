...
CutReady uses Draftline as the only persistence layer for versioned project
...
- Each snapshot captures versioned content: `.sk`, `.sb`, `.md`, supported image
  assets, `.cutready/screenshots/`, and `.cutready/visuals/`
- Runtime paths such as `.chats`, `.cutready/recordings/`,
  `.cutready/agent-state.db`, `.cutready/memory.json`, and
  `.cutready/locks.json` are excluded by policy
