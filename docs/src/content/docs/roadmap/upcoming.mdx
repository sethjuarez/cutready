...
- ✅ **Version history** — Draftline-backed snapshots with visual timeline graph and branching
...
- ✅ **Remote sync** — Push/pull/sync with GitHub remotes via Draftline
...
- 🧪 **Experimental recording** — Hidden behind Settings → Experimental; captures browser action traces and screen/camera/audio takes with teleprompter support

## Experimental: Recording <Badge text="Experimental" variant="note" />

**Goal:** Capture browser interaction sessions and screen/camera/audio takes,
then turn them into clean, editable scripts.
...
- Review multiple browser-action takes without relaunching the browser
- Start screen/camera/audio takes from sketches or storyboards
- Use a teleprompter generated from sketch rows while recording
...
**Goal:** Automated replay and edit-ready video production.

Planned capabilities:

- Controlled browser replay of the finalized action sequence
- Promotion of local recording takes into organized output packages
- Track assembly for screen, camera, system audio, and narration assets
