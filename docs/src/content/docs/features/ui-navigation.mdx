...
- **Move sidebar**: Click the **LayoutGrid** button (leftmost panel toggle in the title bar) to flip the sidebar between left and right
...
- **Debug** — captures JavaScript console output and Rust backend log messages
  for diagnostics
- **Terminal** — opens a system terminal in the project directory (Windows Terminal
  or PowerShell on Windows, Terminal.app on macOS, gnome-terminal/konsole/xterm on Linux)
