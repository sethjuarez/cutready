...
- If you use Azure OAuth, your signed-in user must have access to both the
  Foundry project and the backing model resource:
  - **Azure AI Foundry User** on the Foundry project
  - **Cognitive Services OpenAI User** on the Azure AI Services/OpenAI resource
...
5. **Choose authentication**
   ...
   The signed-in user must have the required Azure RBAC roles. For a shared
   Foundry project, grant **Azure AI Foundry User** on the project and
   **Cognitive Services OpenAI User** on the backing Azure AI Services/OpenAI
   resource. Project access alone may let the user see the project but still
   fail model calls.
...
No special endpoint configuration is needed — just paste the Foundry endpoint and
CutReady handles the Foundry API paths. If you use Azure OAuth, your signed-in
account still needs the RBAC roles listed in the prerequisites above.
...
For shared Azure OpenAI resources using Azure OAuth, make sure your signed-in
user has **Cognitive Services OpenAI User** on the Azure OpenAI resource so
CutReady can invoke deployed models.
