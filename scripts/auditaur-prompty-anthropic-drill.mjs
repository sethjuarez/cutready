// Auditaur Prompty Anthropic battle drill.
//
// Runs real chat turns against a fresh CutReady project and asserts that the
// effective runtime provider is direct Anthropic through the Prompty harness.
// The drill intentionally does not seed API keys. Configure Anthropic in the app
// settings first, set it as the default provider, and keep the Prompty engine
// selected.
//
// Run: npm run test:e2e:harness-prompty-anthropic
...
const timeoutSeconds = Number(process.env.CUTREADY_AUDITAUR_ANTHROPIC_TIMEOUT ?? "360");
const defaultTurnTimeoutMs = Number(process.env.CUTREADY_AUDITAUR_ANTHROPIC_TURN_TIMEOUT ?? "240000");
const reportPath = resolve(
  process.env.CUTREADY_AUDITAUR_ANTHROPIC_REPORT ??
    "target/auditaur-prompty-anthropic-battle-report.json",
);
const projectRoot = mkdtempSync(join(tmpdir(), "cutready-anthropic-battle-"));
...
try {
  ...
  console.log("Starting CutReady under Auditaur observation (Prompty Anthropic battle drill)...");
  launcher = spawn(
    "auditaur",
    [
  ...
  await assertEffectiveAnthropic();

  await runScenario({
    id: "read-and-delegate",
    prompt:
      "You are Writer. Do this exactly once: read anthropic-brief.md, read anthropic-battle.sk, and call delegate_to_agent one time for a concise review. Do not repeat any successful tool call. After the delegated result returns, immediately reply with ANTHROPIC-DELEGATE-4242 and stop.",
  ...
  await runScenario({
    id: "mutate-sketch",
    prompt:
      "You are Writer. Do this exactly once: call update_planning_row, not write_sketch, to update anthropic-battle.sk row 1 narrative so it includes ANTHROPIC-MUTATION-4242. Then read anthropic-battle.sk once. Do not repeat any successful tool call. Reply when persisted and stop.",
    turnTimeoutMs: 180_000,
  });
  ...
  await runScenario({
    ...
    prompt:
      "You are Writer. Do this exactly once: update anthropic-battle.sb description to include ANTHROPIC-STORYBOARD-4242, set one valid simple Elucim v2 visual on row 2 of anthropic-battle.sk, then review that visual once. Do not repeat any successful tool call. Reply with ANTHROPIC-VISUAL-4242 and stop.",
  ...
  writeReport();
  if (failed.length === 0) {
    console.log(`\nPrompty Anthropic battle drill PASSED. Report: ${reportPath}`);
  } else {
    console.error(`\nPrompty Anthropic battle drill FAILED (${failed.length} assertion(s)). Report: ${reportPath}`);
  ...
  report.finishedAt = new Date().toISOString();
  writeReport();
  console.error(`\nPrompty Anthropic battle drill errored. Report: ${reportPath}`);
  console.error(error);
  process.exitCode = 1;
} finally {
  stopSpawnedApp();
...
async function assertEffectiveAnthropic() {
  ...
  const ok = typeof value?.title === "string" &&
    value.title.includes("Anthropic") &&
    typeof value?.text === "string" &&
    value.text.includes("claude-");
  recordAssertion(
    "model-picker-shows-anthropic",
    ok ? "passed" : "failed",
    value,
    ok ? null : "set Anthropic as the default provider before running this drill",
  );
  if (!ok) throw new Error("chat UI is not showing an effective Anthropic model");
}
...
function assertFixtureMutations() {
  const sketch = JSON.parse(readFileSync(join(projectRoot, "anthropic-battle.sk"), "utf8"));
  const storyboard = JSON.parse(readFileSync(join(projectRoot, "anthropic-battle.sb"), "utf8"));
  const visualRefs = (sketch.rows ?? []).map((row) => row.visual).filter(Boolean);
  const mutationOk = JSON.stringify(sketch).includes("ANTHROPIC-MUTATION-4242");
  const storyboardOk = JSON.stringify(storyboard).includes("ANTHROPIC-STORYBOARD-4242");
...
async function runCancellationScenario() {
  ...
  try {
    ...
    await drive("fill", {
      ...
      value:
        "Read anthropic-brief.md, then write an extremely long, exhaustively detailed 2000-word narration script for a product demo about relational databases. Keep going with as much detail as possible.",
    });
    await drive("click", { selector: sel.send });
    if (!await pollExists(sel.stop, true, 20_000)) throw new Error("turn did not start");
    await delay(2500);
...
function assertDurableRunState() {
  ...
  recordAssertion(
    "runs-are-prompty-anthropic",
    runsOk ? "passed" : "failed",
    { completedRuns },
    runsOk ? null : "completed runs were not all direct Anthropic Prompty runs",
  );
...
function writeProjectFixture() {
  ...
  writeFileSync(
    join(projectRoot, "anthropic-brief.md"),
    "# Anthropic Battle Brief\n\nExercise Prompty direct Anthropic with reading, delegation, mutation, storyboard updates, and visual tools.\n",
  );
  writeFileSync(
    join(projectRoot, "anthropic-battle.sk"),
    `${JSON.stringify({
      title: "Anthropic Battle Sketch",
      description: "Fixture sketch for direct Anthropic Prompty battle testing.",
      rows: [
        {
          time: "~10s",
  ...
  writeFileSync(
    join(projectRoot, "anthropic-battle.sb"),
    `${JSON.stringify({
      title: "Anthropic Battle Storyboard",
      description: "Storyboard fixture.",
      items: [{ type: "sketch_ref", path: "anthropic-battle.sk" }],
      created_at: now,
      updated_at: now,
    }, null, 2)}\n`,
  );
...
function ensureNoActiveCutReady() {
  ...
  if (
    ...
    throw new Error(
      `An active CutReady Auditaur session is already running (pid ${app.pid}, session ${app.sessionId}). Close it before running the Anthropic battle drill.`,
    );
  }
}
...
function removeProjectFromRecents() {
  ...
  try {
    ...
    store.recent_projects = store.recent_projects.filter(
      (project) => !String(project?.path ?? "").includes("cutready-anthropic-battle-"),
    );
