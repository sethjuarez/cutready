/**
 * SlidesView — the rich slide-preview mode: screenshot/visual + narrative +
 * actions side panel. Extracted from the original SketchPreview body. Pure
 * with respect to the shell: receives slides + currentIdx + setters via props
...
export function SlidesView({
  ...
  // Keyboard navigation. Esc closes, ←/→ navigate, T = teleprompter, S = slide-only.
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === "Escape") onClose();
        ...
        onSwitchMode("slide-only");
      }
    };
    window.addEventListener("keydown", handler);
    return () => window.removeEventListener("keydown", handler);
  }, [onClose, goNext, goPrev, total, onSwitchMode, setCurrentIdx]);
    ...
    <div className="fixed inset-0 z-modal bg-[rgb(var(--color-surface))] flex flex-col select-none">
      ...
      <div className="flex items-center justify-between px-6 py-3 shrink-0 border-b border-[rgb(var(--color-border))]">
        ...
        <div className="flex items-center gap-1">
          ...
          {/* Slide-only mode */}
          <button
            onClick={() => onSwitchMode("slide-only")}
            className="flex items-center gap-1.5 text-xs text-[rgb(var(--color-text-secondary))] hover:text-[rgb(var(--color-accent))] transition-colors px-2 py-1 rounded-md hover:bg-[rgb(var(--color-surface-alt))]"
            title="Slide-only mode (S)"
          >
        ...
        <div className="flex items-center gap-1.5">
          {slides.map((s, i) => (
            <button
              ...
              title={s.type === "title" ? s.heading : `Slide ${i + 1}`}
            />
          ))}
        </div>
