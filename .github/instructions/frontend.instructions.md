...
- Auditaur wraps the normal Tauri runner; it does not replace it. Ensure dependencies are installed (`npm ci`) before wrapper mode, then start via `auditaur debug --app cutready --active --require-frontend --json run --timeout-seconds 180 -- npm run debug`. `npm run debug` expands to `tauri dev`; `npm run tauri -- dev` is the explicit equivalent.
