...
- For app smoke tests, launch the normal Tauri dev command through Auditaur rather than invoking Auditaur as a replacement runtime. Run `npm ci` first when `node_modules` is missing, then wrap `npm run debug` with `auditaur debug --app cutready --active --require-frontend --json run --timeout-seconds 180 -- npm run debug`.
